...
class TicketAnalyzerv2:
    ...
    def get_tfidf_embeddings(self, texts):
        ...
        Returns:
            ...
            TfidfVectorizer or CountVectorizer: The fitted vectorizer for future reference
        """
        # Special handling for extremely small datasets
        num_docs = len(texts)
        
        # For extremely small datasets (1-2 documents), use CountVectorizer instead
        if num_docs <= 2:
            print(f"Dataset too small ({num_docs} documents) for TF-IDF. Using CountVectorizer instead.")
            vectorizer = CountVectorizer(
                max_features=self.max_features,
                ngram_range=(1, 1)  # Use only unigrams for very small sets
            )
            count_matrix = vectorizer.fit_transform(texts)
            return count_matrix.toarray(), vectorizer
        
        # For normal datasets, use TF-IDF with appropriate parameters
        ngram_range = (1, 2) if self.use_bigrams else (1, 1)
        
        # Dynamically adjust min_df and max_df based on number of documents
        if num_docs < 5:
            min_df = 1
            max_df = 1.0
        elif num_docs < 10:
        ...
        print(f"Using TF-IDF with min_df={min_df}, max_df={max_df} for {num_docs} documents")
        
        vectorizer = TfidfVectorizer(
            max_features=self.max_features,
            ngram_range=ngram_range,
            min_df=min_df,      # Dynamically adjusted
            max_df=max_df,      # Dynamically adjusted
        ...
        print(f"Generated embeddings with {tfidf_matrix.shape[1]} features")
